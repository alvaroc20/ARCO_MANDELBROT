		...
		#pragma omp for schedule(static)
			for(i = 0; i < height; i++){
				for(j = 0; j < width; j++) {
					x = xmin + j/k; 
					y = ymax - i/k; 
					aux[i][j] = mandel_val(x, y, MAX_ITER);
				}
                        }
		}
...
//ÁLVARO CERDÁ PULLA
// Procesador Intel I7 de 8 núcleos

/*	
	Para la prueba con el cuadrado y el rectangulo especificada en Campus Virtual los resultados son los siguientes:
	- ORIGINAL: cuadrado 13.18s, rectangulo 5.24s
	- PARALELO sin schedule: cuadrado 15.08s, rectangulo 6.06s
	- PARALELO con schedule(static): cuadrado 2.62s, rectangulo 1.54s. Si ponemos un numero tt (probados 5,7 y 15) saldrían 2.02, es decir, no ponerlo aumenta el tiempo.
	- PARALELO con schedule(dynamic): cuadrado 1.98s, rectangulo 0.85s. Si usase un numero tt aumentaria a algo mas de 2s el tiempo del cuadrado.
	- Una vez usado el diff -qs compruebo que las dos imagenes son identicas
*/